...
通知触发相关的 core trace 会统一使用 `notify_trigger` 这个事件名。它表示“通知模块产生了一条记录”，具体是启动、跳过、命中还是提交工作流失败，需要继续看同一条记录里的其它字段。
...
| 字段 | 说明 |
| --- | --- |
| `final_task` | 最终提交给执行系统的任务描述。 |
| `task_id` | 通知模块记录不属于某次任务执行内部，所以这里通常为空。通知真正提交后，工作流执行阶段会产生自己的任务流程 trace。 |

如果你看到带 `final_task` 的通知记录，但后续没有任务流程 trace，通常需要检查 core 是否正在运行、当前是否已有任务占用队列、工作流是否存在且触发器已启用。
