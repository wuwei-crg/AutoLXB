...
使用手册包含快速任务、任务模板、工作流、路线编辑、便携包导入导出、配置说明和 Trace 排查等完整内容。
...
AutoLXB 适合“重复、线性、可触发”的任务。任务页主要分为：

- **快速任务**：立即执行一次，用来试跑任务描述和设备配置。
- **任务模板**：保存可复用任务，维护目标应用、用户操作文档、TASK_DECOMPOSE 开关和一条主路线。
- **工作流**：按顺序编排一个或多个模板，并配置可选触发方式。触发方式可以为空、定时触发或通知触发。
...
如果不确定任务描述是否稳定，可以先用 **快速任务** 试跑一次。稳定后创建 **任务模板**；需要定时或通知触发时，再把模板加入 **工作流** 并配置触发方式。

### 5. 可选：保存任务路线

对于经常重复执行的任务模板，可以开启任务路线。模板执行后，在路线编辑页保留有用步骤、删除无关动作，然后点击 **手动保存路线**。后续执行会优先按该模板路线跳转，减少视觉模型调用和不确定性。

## 示例任务

[`sample_tasks`](sample_tasks/) 目录提供了几份可在 AutoLXB 任务页导入的旧格式任务 JSON 示例。当前导入器会兼容迁移到任务模板 / 工作流结构：

- `baidu-tieba-one-click-sign-in.json`：百度贴吧一键签到示例。
- `bilibili-text-post.json`：Bilibili 图文动态发布示例。
...
任务运行时会进入状态机，先尝试命中当前模板自己的已保存路线；如果没有可用路线，或路线回放不能完成任务，再降级到视觉执行。
